...
# (6) System task completion
TASK_COMPLETION = "system_task_completion"
FOLLOWUP_TASK_COMPLETION = "system_followup_task_completion"


NAN_VAL = "NA"
TASK_COMPLETION_METRICS = (TASK_COMPLETION, FOLLOWUP_TASK_COMPLETION)
...
def compare_slot_values(slot_values_ref, slot_values_hyp, service, use_fuzzy_match):
    ...
    Returns:
      ...
      list_cor: list of correctness scores, each corresponding to one slot in the
...
def get_system_task_completion(
    frame_ref: dict[str, Any],
    frame_hyp: dict[str, Any],
    service_schema: dict[str, Any],
    use_fuzzy_match: bool,
) -> dict[
    Literal["system_task_completion", "system_followup_task_completion"],
    float | str,
]:
    metadata = frame_ref["metadata"]
    if not metadata["api_call_required"]:
        return {TASK_COMPLETION: NAN_VAL, FOLLOWUP_TASK_COMPLETION: NAN_VAL}
    # require intent to be correct for task completion
    ref_intent, hyp_intent = (
        frame_ref["state"]["active_intent"],
        frame_hyp["state"]["active_intent"],
    )
    if ref_intent != hyp_intent:
        metrics = {
            TASK_COMPLETION: 0.0,
            FOLLOWUP_TASK_COMPLETION: 0.0 if metadata["followup_task"] else NAN_VAL,
        }
        return metrics
    # remove slots that are not relevant to  the current
    # task from the dialogue state
    frame_ref = deepcopy(frame_ref)
    frame_hyp = deepcopy(frame_hyp)
    all_slots = set(frame_ref["state"]["slot_values"].keys()).union(
        frame_hyp["state"]["slot_values"].keys()
    )
    for s in {s for s in all_slots if s not in metadata["call_parameters"]}:
        for frame_ in (frame_ref, frame_hyp):
            if s in (ref_slot_vals := frame_["state"]["slot_values"]):
                ref_slot_vals.pop(s)
    goal_accuracy = get_average_and_joint_goal_accuracy(
        frame_ref, frame_hyp, service_schema, use_fuzzy_match
    )
    jga = goal_accuracy[JOINT_GOAL_ACCURACY]
    metrics = {
        TASK_COMPLETION: jga,
        FOLLOWUP_TASK_COMPLETION: jga if metadata["followup_task"] else NAN_VAL,
    }
    return metrics
