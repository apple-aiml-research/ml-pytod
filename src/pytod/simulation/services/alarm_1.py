...
@register_command(service="Alarm_1")
class GetAlarms(SearchCommand):
    def __init__(self, dialogue_id: DialogueID):
        super().__init__(dialogue_id)


@register_command(service="Alarm_1")
class AddAlarm(ConfirmedCommand):
    new_alarm_time: ConfirmedCommandArgument[str] = ConfirmedCommandArgument()
    new_alarm_name: ConfirmedCommandArgument[str] = ConfirmedCommandArgument()
