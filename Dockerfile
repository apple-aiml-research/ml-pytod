...
# and `pip install pytod` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "pytod", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n pytod -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "pytod", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "pytod", "YOUR_CONSOLE_SCRIPT"]
