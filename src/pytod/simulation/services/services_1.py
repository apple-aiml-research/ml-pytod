...
@register_command(service="Services_1")
class FindProvider(SearchCommand):
    city: SearchCommandArgument[str] = SearchCommandArgument()
    is_unisex: SearchCommandArgument[str] = SearchCommandArgument()
...
class BookAppointment(ConfirmedCommand):
    stylist_name: ConfirmedCommandArgument[str] = ConfirmedCommandArgument()
